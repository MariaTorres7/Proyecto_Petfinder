...
export const ValidarUsuario = async (req, res) => {
    try {
      ...
      let sql = `select id_user, fullname from users where email = '${email}' and password = '${password}'`;
      let [rows] = await pool.query(sql);
  
      if (rows.length > 0) {
        let token = jswt.sign({ user: rows[0] }, process.env.SECRET, { expiresIn: process.env.TIME });
        return res.status(200).json({ "message": "Usuario autorizado", "token": token, "fullname": rows[0].fullname });
      } 
    ...
    try {
        ...
        if (!token_cliente) {
            ...
            let decode = jswt.verify(token_cliente, process.env.SECRET, (error, decoded) => {
                if (error) {
                    return res.status(402).json({"message": "token invalido"});
                } else {
